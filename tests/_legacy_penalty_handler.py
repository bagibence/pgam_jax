"""
Frozen snapshot of the pre-refactor (dispatch-style) PenaltyHandler.

TEMPORARY migration-only parity oracle. This is a verbatim copy of
``src/pgam_jax/_penalty_handler.py`` as of commit 8363adb (the SqrtMethod /
_Penalty dataclass implementation), with the only edit being an absolute
import of ``_slam_compute``. It exists so the equinox class-hierarchy rewrite
can be compared against the old numerics at the Commit 2 audit point. Deleted
in Commit 3 (Task 8); do not build on it.
"""
